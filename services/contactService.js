...
const consolidateContacts = async (email, phoneNumber) => {
  ...
  if (!primaryContact) {
    ...
    const primaryContactIds = [...new Set(matchingContacts.map(c => c.linkedId))];
  ...
  // Check if we need to create a new contact or convert primary to secondary
  const hasEmail = allLinkedContacts.some(c => c.email === email);
  const hasPhone = allLinkedContacts.some(c => c.phoneNumber === phoneNumber);
  
  let newContact = null;
  
  if ((email && !hasEmail) || (phoneNumber && !hasPhone)) {
    // Check if we have a contact that matches the new info partially
    const existingPartialMatch = matchingContacts.find(c => 
      (c.email === email && c.phoneNumber !== phoneNumber) || 
      (c.phoneNumber === phoneNumber && c.email !== email)
    );
    
    if (existingPartialMatch && existingPartialMatch.linkPrecedence === 'primary') {
      // Convert this primary to secondary and link to the oldest primary
      existingPartialMatch.linkPrecedence = 'secondary';
      existingPartialMatch.linkedId = primaryContact._id;
      existingPartialMatch.updatedAt = new Date();
      await existingPartialMatch.save();
      
      // Update all contacts linked to this one to now link to the primary
      await Contact.updateMany(
        { linkedId: existingPartialMatch._id, deletedAt: null },
        { $set: { linkedId: primaryContact._id, updatedAt: new Date() } }
      );
    } else if (!existingPartialMatch) {
